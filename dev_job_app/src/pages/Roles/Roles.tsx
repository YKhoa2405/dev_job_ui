...
const Roles = () => {
    ...
    const handleEditRole = (role: IRole) => {
        ...
        const initializedPermissions = permissionsData.map((group: PermissionGroup) => ({
            ...
            permissions: group.permissions.map((perm: any) => ({
                ...perm,
                enabled: role.permissions.some((rolePerm: any) => rolePerm._id === perm._id),
            })),
        }));
        setEditForm({
            name: role.name,
                                            ...
                                            <div key={group.group} className="mb-2">
                                                <div className="flex items-center justify-between bg-gray-100 p-2 rounded cursor-pointer">
                                                    ...
                                                    {expandedGroups.includes(group.group) ? (
                                                        <ChevronUp size={16} onClick={() => toggleGroup(group.group)} />
                                                    ) : (
                                                        <ChevronDown size={16} onClick={() => toggleGroup(group.group)} />
                                                    )}
                                                </div>
                                                {expandedGroups.includes(group.group) && (
                                                    <div className="mt-2 grid grid-cols-2 gap-2">
                                                        {group.permissions.map((perm, permIndex) => (
                                                            <div key={perm.apiPath} className="mb-2 border rounded p-2 bg-white">
                                                                ...
                                                                <div className="mt-1 text-sm font-mono">
                                                                    <span
                                                                        className={`mr-1 font-bold ${perm.method === 'get' || perm.method === 'GET'
                                                                            ? 'text-green-600'
                                                                            : perm.method === 'post' || perm.method === 'POST'
                                                                                ? 'text-blue-600'
                                                                                : perm.method === 'patch' || perm.method === 'PATCH'
                                                                                    ? 'text-yellow-600'
                                                                                    : perm.method === 'delete' || perm.method === 'DELETE'
                                                                                        ? 'text-red-600'
                                                                                        : 'text-gray-600'
                                                                            }`}
                                                                    >
